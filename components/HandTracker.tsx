...
const HandTracker = ({ autoStart = true, onPhrase, voiceId, modelId }: HandTrackerProps) => {
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto bg-slate-900 rounded-3xl overflow-hidden border border-slate-800 shadow-2xl">
      ...
      {/* Camera controls */}
      <div className="absolute left-2 top-2 right-2 sm:left-3 sm:top-3 sm:right-auto z-20 flex flex-wrap sm:flex-nowrap items-center justify-between sm:justify-start gap-2 rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-white backdrop-blur-md">
        <div className="flex items-center gap-2 text-xs font-semibold text-white/80 shrink-0">
        ...
        <select
          ...
          className="flex-1 min-w-[120px] max-w-xs sm:max-w-[220px] rounded-lg border border-white/10 bg-black/30 px-2 py-1 text-[16px] sm:text-xs text-white/90 outline-none disabled:opacity-60"
        ...
        {!isCameraActive ? (
          <button
            ...
            className="shrink-0 rounded-lg bg-white px-2.5 py-1 text-xs font-bold text-black disabled:opacity-70"
          >
          ...
          <button
            type="button"
            onClick={stopCamera}
            className="shrink-0 inline-flex items-center gap-1 rounded-lg bg-white/10 px-2.5 py-1 text-xs font-bold text-white hover:bg-white/15 transition-colors"
            title="Stop camera"
          >
